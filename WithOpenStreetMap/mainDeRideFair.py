...
class EligibilityRiderMatrix:
    ...
    def select_driver(self, eligible_drivers, drivers, rider, DP_assigned, output_file):
        if len(eligible_drivers) == 1:
            output_file.write(f"Only one eligible driver: d{eligible_drivers[0]+1}\n")
            return eligible_drivers[0]
        
        # Step 1: Determine each eligible driver's current load
        driver_loads = {}
        for driver_idx in eligible_drivers:
            driver_id = drivers[driver_idx].id
            current_load = len(DP_assigned[driver_id]['riders'])
        ...
        # Step 2: Group eligible drivers by load in ascending order
        load_groups = {}
        for driver_idx, load in driver_loads.items():
            if load not in load_groups:
                load_groups[load] = []
            load_groups[load].append(driver_idx)
        print("Load groups: {load_groups}")
        sorted_loads = sorted(load_groups.keys())
        output_file.write(f"Load groups: {load_groups}, sorted loads: {sorted_loads}\n")
        
        # Step 3: For each load group (starting from smallest load)
        for load in sorted_loads:
            Lg = load_groups[load].copy()  # Make a copy to avoid modifying original
            output_file.write(f"Processing load group {load} with drivers: {[d+1 for d in Lg]}\n")
            
            # Step 4: While Lg is not empty
            while Lg:
                # Step 5: Select driver from group
                if len(Lg) > 1:
                    d = random.choice(Lg)
                    output_file.write(f"Randomly selected driver d{d+1} from group: {[dr+1 for dr in Lg]}\n")
                else:
                    d = Lg[0]
                    output_file.write(f"Selected only driver d{d+1} from group\n")
                
                # Step 6: Simulate adding the new request to driver's current route
                driver = drivers[d]
                driver_id = driver.id
                
                # Calculate the updated route length with the new rider
                updated_route_length = self.calculate_updated_route_length(driver, rider, DP_assigned, output_file)
                
                # Calculate M_Pd (max route length allowed for driver d)
                if driver.threshold == 0:
                    # For threshold 0, we use the original shortest path as the maximum
                    original_sp, original_sp_length = self.shortest_path_distance(driver.source, driver.destination, output_file)
                    M_Pd = original_sp_length
                else:
                    # For non-zero threshold, calculate based on original route
                    original_sp, original_sp_length = self.shortest_path_distance(driver.source, driver.destination, output_file)
                    M_Pd = original_sp_length * (1 + (driver.threshold / 100))
                
                output_file.write(f"Driver d{d+1}: updated_route_length={updated_route_length}, M_Pd={M_Pd}\n")
                
                # Step 7: Check if route constraint is satisfied
                if updated_route_length <= M_Pd:
                    output_file.write(f"Route constraint satisfied for driver d{d+1}. Assigning rider.\n")
                    return d
                else:
                    output_file.write(f"Route constraint NOT satisfied for driver d{d+1}. Removing from group.\n")
                    Lg.remove(d)
        
        # If no driver satisfies the constraint, fall back to original selection logic
        output_file.write("No driver satisfies route constraints. Falling back to original selection logic.\n")
        max_seats = -1
        ...
        output_file.write(f"Selected driver d{selected_driver+1} from drivers with max seats: {[d+1 for d in drivers_with_max_seats]}\n")
